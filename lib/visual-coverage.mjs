...
export function assessVisualCoverage(scenes,duration){
 ...
 const mediaSeconds=scenes.filter(scene=>scene.asset).reduce((sum,scene)=>sum+seconds(scene),0);
 const videoScenes=scenes.filter(scene=>scene.asset?.kind==='video');
 const photoScenes=scenes.filter(scene=>scene.asset?.kind==='image');
 ...
 return {
  ...
  photoSeconds:Number(photoSeconds.toFixed(2)),videoScenes:videoScenes.length,photoScenes:photoScenes.length,mapScenes:scenes.filter(scene=>scene.map).length,
  missing,
