...
export async function automatic(s,j,{root,dir,log,store}){
 ...
 if(reviewed.pending.length)throw Error('Mídia real ausente em '+reviewed.pending.map(item=>item.id).join(', ')+'. Produção pausada antes das animações para evitar cenas vazias.');
 ...
 j.auto.mediaCoverage={seconds:coverage.seconds,ratio:coverage.ratio,videoSeconds:coverage.videoSeconds,videoRatio:coverage.videoRatio,photoSeconds:coverage.photoSeconds,videoScenes:coverage.videoScenes,photoScenes:coverage.photoScenes,mapScenes:coverage.mapScenes};
 log(j,'Mídias selecionadas: '+coverage.videoScenes+' vídeos, '+coverage.photoScenes+' fotografias reais e '+coverage.mapScenes+' mapas; '+Math.round(coverage.ratio*100)+'% do tempo com mídia real.');
