...
export async function automatic(s,j,{root,dir,log,store}){
 ...
 const videoSeconds=editorialScenes.filter(scene=>scene.asset?.kind==='video').reduce((sum,scene)=>sum+scene.end-scene.start,0),videoRatio=duration?videoSeconds/duration:0;
 j.auto.mediaCoverage={seconds:Number(mediaSeconds.toFixed(2)),ratio:Number(mediaRatio.toFixed(3)),videoSeconds:Number(videoSeconds.toFixed(2)),videoRatio:Number(videoRatio.toFixed(3))};
 if(duration>=30&&editorialScenes.length>=5&&(mediaRatio<.8||videoRatio<.75))throw Error('Cobertura insuficiente de vídeo real ou fotografia: exigidos 80% de mídia real e pelo menos 75% de vídeo neste formato. Produção pausada antes das animações para rever a busca de mídia.');
