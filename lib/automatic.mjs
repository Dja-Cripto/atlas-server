...
export async function automatic(s,j,{root,dir,log}){
 ...
 for(const direction of plan){
  ...
  if(scene.kind==='map'){
   ...
   const mainGeometry=g=>{if(g.type!=='MultiPolygon')return g;const area=polygon=>Math.abs(polygon[0].reduce((n,p,i,a)=>{const q=a[(i+1)%a.length];return n+p[0]*q[1]-q[0]*p[1];},0));return {type:'Polygon',coordinates:g.coordinates.slice().sort((a,b)=>area(b)-area(a))[0]};};
 ...
 await save('resolved.json',reviewed.scenes);
 
 const mediaSeconds=reviewed.scenes.filter(scene=>scene.asset).reduce((sum,scene)=>sum+scene.end-scene.start,0),mediaRatio=duration?mediaSeconds/duration:0;
 j.auto.mediaCoverage={seconds:Number(mediaSeconds.toFixed(2)),ratio:Number(mediaRatio.toFixed(3))};
 
 const composed=composeContinuity(reviewed.scenes);
