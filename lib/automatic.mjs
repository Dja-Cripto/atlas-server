 ...
 try {
  ...
  await renderMedia({
   ...
   onProgress:({progress,renderedFrames,encodedFrames})=>{
    ...
    const frameInfo=totalFrames>0?` (${renderedFrames}/${totalFrames} quadros)`:'';
    j.auto.renderingMp4=true;
    j.auto.renderProgress=pct;
    j.auto.renderedFrames=renderedFrames||0;
    j.auto.totalFrames=totalFrames;
    j.auto.liveStatus=`Renderizando MP4 1080p: ${pct}%${frameInfo} · 4 núcleos`;
    const now=Date.now();
    ...
    if(pct>=last+5||pct===100){
     last=pct;
     log(j,`Renderização MP4: ${pct}%${frameInfo}.`);
    }
   }
  });
