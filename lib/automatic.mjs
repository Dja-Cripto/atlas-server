...
export async function automatic(s,j,{root,dir,log}){
 ...
 while(true){
  try{
   j.auto.previewValidation=await validatePreview(entryPoint,units,root,(checked,total)=>{j.auto.validationResumeIndex=checked;j.auto.progress=86+Math.round(checked/total*5);if(checked%5===0||checked===total)log(j,`Validação das cenas: ${checked}/${total}.`);},j.auto.validationResumeIndex||0);
   break;
  }catch(error){
