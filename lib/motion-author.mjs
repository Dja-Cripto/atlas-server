...
export async function authorMotion(s,j,manifest,{root,log,repairError,repairSceneIndexes=[]}){
 ...
 const children=scenes.map(scene=>{const needsBackdrop=Boolean(scene.map||!scene.asset);return `<Sequence from={${scene.from}} durationInFrames={${scene.durationInFrames}}><AbsoluteFill>${needsBackdrop?`<SceneBackdrop scene={scenes[${scene.index}]} backgroundScene={Number.isInteger(scenes[${scene.index}].backgroundIndex)?scenes[scenes[${scene.index}].backgroundIndex]:null} duration={${scene.durationInFrames}}/>`:''}<Scene${scene.index} scene={scenes[${scene.index}]} context={contexts[${scene.index}]} /></AbsoluteFill></Sequence>`;}).join('');
    ...
    const vol=balancedMusic?0.24:(j.selectedMusic?.defaultVolume||0.06);
    try{
...
export async function authorShortsMotion(s,j,manifest,{root,log,shortIndex}){
 ...
 try{
  ...
  bgmTag=`<Loop durationInFrames={1800}><Audio src={staticFile("${shortBgmRel}")} volume={${j.productionVersion==='v3'&&j.shorts?.musicNormalization?.[shortIndex]?0.20:0.06}} /></Loop>`;
 }catch{
  try{
   await access(path.join(root,'renderer/public/audio/bgm-ambient.mp3'));
