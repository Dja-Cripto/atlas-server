...
PHOTOS & ARCHIVAL IMAGES: A photo MUST do more than zoom with a caption. Animate a subject-aware reveal, meaningful crop comparison, traced detail, masked archival layer or researched annotation tied to the spoken sentence. Use objectFit cover and continuous restrained camera motion, but the photograph's role and explanatory treatment must differ from a clean video shot. For historical images, identify date or provenance only when verified; evoke the period through restrained visual treatment, never fake a historical document or place. NEVER leave photos static, letterboxed with black empty margins, or distorted. Keep the image continuously moving through the scene without looping or a frozen hold. Never use a bright full-frame exposure flash, white wash, mixBlendMode screen/color-dodge/plus-lighter, or any opacity above 1; these effects destroy image detail.
...
export function generateFallbackSceneCode(scene,isShort=false){
 if(scene?.kind==='diagram'&&/heat|hot|cold|cool|winter|summer|expand|contract|thermal|atom/i.test([scene.title,scene.narration].join(' '))){
  return `import React from 'react';
import {AbsoluteFill,interpolate,useCurrentFrame} from 'remotion';
export default function MotionScene({scene}: {scene:any;context?:any}) {
 const frame=useCurrentFrame();
 const dur=Math.max(1,scene.durationInFrames||90);
 const cold=/cold|cool|winter|contract/i.test(scene.narration||'');
 const progress=interpolate(frame,[0,dur],[0,1],{extrapolateLeft:'clamp',extrapolateRight:'clamp'});
 const spacing=cold?158-36*progress:122+36*progress;
 const energy=cold?1-progress:progress;
 const dots=Array.from({length:12},(_,i)=>({x:i%4,y:Math.floor(i/4)}));
 return <AbsoluteFill style={{background:'#122736',color:'#f7eee0',fontFamily:'Arial,sans-serif',overflow:'hidden'}}>
  <div style={{position:'absolute',left:110,top:95,fontSize:58,fontWeight:800}}>{cold?'COLD CONTRACTS METAL':'HEAT EXPANDS METAL'}</div>
  <div style={{position:'absolute',left:110,top:178,fontSize:27,letterSpacing:3,color:'#e6b977'}}>IRON AT THE ATOMIC SCALE · SCHEMATIC</div>
  <div style={{position:'absolute',left:550,top:355,width:850,height:460,border:'2px solid rgba(247,238,224,.35)',borderRadius:24}}>
   {dots.map((dot,i)=><div key={i} style={{position:'absolute',left:70+dot.x*spacing,top:75+dot.y*spacing,width:42,height:42,borderRadius:'50%',background:cold?'#8ecce8':'#f1ad5a',boxShadow:'0 0 '+(12+24*energy)+'px '+(cold?'#8ecce8':'#f1ad5a'),transform:'translate('+(Math.sin(frame*.17+i)*energy*7)+'px,'+(Math.cos(frame*.15+i)*energy*7)+'px)'}}/>)}
  </div>
  <div style={{position:'absolute',left:110,bottom:112,fontSize:31,color:'#f7eee0'}}>{cold?'LESS MOTION  →  CLOSER TOGETHER':'MORE MOTION  →  FARTHER APART'}</div>
 </AbsoluteFill>;
}`;
 }
 if(scene?.kind==='diagram'&&/oppos|two directions|two currents|inflow|outflow/i.test([scene.title,scene.narration].join(' '))){
  return `import React from 'react';
import {AbsoluteFill,interpolate,useCurrentFrame} from 'remotion';
export default function MotionScene({scene}: {scene:any;context?:any}) {
 const frame=useCurrentFrame();
 const dur=Math.max(1,scene.durationInFrames||90);
 const reveal=interpolate(frame,[0,Math.max(1,dur*.38)],[0,1],{extrapolateLeft:'clamp',extrapolateRight:'clamp'});
 const travel=interpolate(frame,[0,dur],[0,520],{extrapolateLeft:'clamp',extrapolateRight:'clamp'});
 return <AbsoluteFill style={{background:'#071f2d',color:'#f1f4ed',fontFamily:'Arial,sans-serif',overflow:'hidden'}}>
  <div style={{position:'absolute',left:110,top:105,fontSize:64,fontWeight:800,maxWidth:1600}}>{scene.heading}</div>
  <div style={{position:'absolute',left:110,top:300,width:1700,height:220,background:'linear-gradient(90deg,#0d5573,#2aa8d0)',borderRadius:20,opacity:reveal}}>
   <div style={{position:'absolute',left:45,top:25,fontSize:30,fontWeight:700}}>SURFACE FLOW</div>
   <div style={{position:'absolute',left:Math.min(1250,260+travel),top:75,fontSize:110,fontWeight:900}}>→</div>
  </div>
  <div style={{position:'absolute',left:110,top:560,width:1700,height:220,background:'linear-gradient(90deg,#735036,#b77e4f)',borderRadius:20,opacity:interpolate(frame,[Math.max(1,dur*.22),Math.max(2,dur*.65)],[0,1],{extrapolateLeft:'clamp',extrapolateRight:'clamp'})}}>
   <div style={{position:'absolute',left:45,top:25,fontSize:30,fontWeight:700}}>DEEP RETURN</div>
   <div style={{position:'absolute',left:Math.max(200,1220-travel),top:75,fontSize:110,fontWeight:900}}>←</div>
  </div>
  <div style={{position:'absolute',left:110,bottom:100,fontSize:28,letterSpacing:3}}>TWO LAYERS · OPPOSITE DIRECTIONS</div>
 </AbsoluteFill>;
}`;
 }
 if(scene?.kind==='diagram')throw Error('Diagrama sem animação explicativa válida; renderização interrompida para não publicar um cartão de planejamento.');
...
export function usesCleanMediaFastPath(scene,repairThisScene=false){return scene.editorialRole==='body'&&scene.asset?.kind==='video'&&['clean','label'].includes(scene.treatment)&&!repairThisScene;}
