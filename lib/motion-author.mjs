...
export default function MotionScene({scene}: {scene:any; context?:any}) {
 ...
 return <AbsoluteFill style={{overflow:'hidden',backgroundColor:'#101817'}}>
  ...
  ${safeLabel?`<div style={{position:'absolute',left:64,bottom:58,maxWidth:'72%',padding:'11px 18px',borderRadius:9,background:'rgba(7,20,18,.68)',boxShadow:'0 10px 32px rgba(0,0,0,.24)',color:'#f5f1e7',fontSize:${labelSize},fontWeight:700,lineHeight:1.04,letterSpacing:'-.015em',opacity:interpolate(frame,[4,12,Math.max(13,duration-12),Math.max(14,duration-4)],[0,1,1,0],{extrapolateLeft:'clamp',extrapolateRight:'clamp'})}}>${safeLabel}</div>`:''}
 </AbsoluteFill>;
}`;
}
