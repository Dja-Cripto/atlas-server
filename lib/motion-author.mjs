...
DOCUMENTARY BOOKENDS: currentScene.editorialRole is opening, openingDevelopment, body or closing. If opening, make the first image immediately specific to the researched subject and turn the spoken hook into a visible question or reveal. If openingDevelopment, continue the first scene's unresolved question through a distinct, researched visual: show the stakes, a revealing comparison or the geographic mechanism, without repeating the same title or card. You may use one large, legible title or place name when it adds meaning, but never force a reusable title-card template, logo or black screen. Let the composition develop during the existing scene duration and hand off clearly to scene two. If closing, visually resolve or echo the opening question through the final researched image or idea, then provide a deliberate final beat (settle, reveal, restrained fade or clean end) during the existing narration/audio tail. Do not finish mid-motion, add a generic subscribe card, or claim the story is resolved when the facts do not support it. These instructions apply only to the long documentary; vertical Shorts start immediately without ceremonial bookends.
