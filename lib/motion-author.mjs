...
export function normalizeMotionCode(code,scene=null){
 ...
 normalized=normalized.replace(/backdropFilter\s*:\s*['"][^'"]*['"]/g,'');
 normalized=normalized.replace(/mixBlendMode\s*:\s*['"][^'"]*['"]/g,'');
...
export function validateMotionCode(code,scene=null){
 ...
 if(scene?.countries?.length){
  ...
  for(const country of scene.countries){
   ...
   if(literal.test(code))throw Error('Nome de país ainda não narrado não pode aparecer em destaque: '+name);
  }
 }
