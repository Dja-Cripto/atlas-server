...
export async function authorMotion(s,j,manifest,{root,log,repairError,repairSceneIndexes=[]}){
 ...
 for(const scene of scenes){
  ...
  for(let attempt=0;attempt<3;attempt++){
   ...
   if(!result){
    ...
    try{
     ...
     result=await generateJSON({...s,sceneProvider:'go',goModel:chosenModel,timeoutMs:attempt===0?motionTimeout():90000},j,prompt);
     ...
     log(j,'Cena '+(scene.index+1)+': '+reason+' em '+motionModelForAttempt(model,attempt)+'; tentativa '+(attempt+1)+'/3.');
     if(attempt===2)break;
  ...
  if(accepted.fallback){fallbackCount++;consecutiveFallbacks++;j.auto.visualFallbacks=[...new Set([...(j.auto.visualFallbacks||[]),`shot-${scene.index+1}`])];}else consecutiveFallbacks=0;
  previousHandoff=accepted.handoff;handoffs.push(previousHandoff);
  j.auto.motion.completed=scene.index+1;j.auto.progress=70+Math.round((j.auto.motion.completed/scenes.length)*15);
  ...
  for(let attempt=0;attempt<3;attempt++){
   ...
   if(!result){
    ...
    try{
     ...
     result=await generateJSON({...s,sceneProvider:'go',goModel:motionModelForAttempt(model,attempt),timeoutMs:attempt===0?motionTimeout():90000},j,(attempt===0?shortsSceneContract:rescueContract)+'\nSCENE_ASSIGNMENT: '+JSON.stringify(attempt===0?assignment:compactRescueAssignment(assignment)));
     await writeFile(path.join(folder,`scene-${scene.index}-attempt-${attempt+1}.json`),JSON.stringify(result,null,2));
    }catch(err){
