...
PHOTOS & ARCHIVAL IMAGES: Every photograph needs visible, restrained motion or a purposeful effect. Choose freely: subject-aware push, pan, reveal, detail highlight, mask or comparison. A simple elegant move is enough when the narration is descriptive; avoid unnecessary text. When narration provides data, age, dates or dimensions, animate that researched fact over the photo. Never leave a photograph static. Preserve image proportions and detail; do not fake documents or provenance. Avoid bright full-frame exposure flashes, white wash, blend modes that destroy detail and opacity above 1.
...
const motionTimeout=()=>Math.min(420000,Math.max(60000,Number(process.env.ATLAS_MOTION_TIMEOUT_MS)||240000));
   ...
   if(!result){
    ...
    try{
     ...
     if(attempt===2){
      ...
      j.auto.visualFallbacks=[...new Set([...(j.auto.visualFallbacks||[]),scene.id])];
