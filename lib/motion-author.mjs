import {rescueContract,motionModelForAttempt,compactRescueAssignment} from './motion-rescue.mjs';
import {narrationNeedsExplanation,factualCaption} from './visual-intent.mjs';
import {diagramFallbackCode} from './diagram-fallback.mjs';
...
export function normalizeMotionCode(code,scene=null){
 ...
 let normalized=code
  ...
  .replace(/(['\"])(\d+(?:\.\d+)?px\s+(?:solid|dashed|dotted)\s+)\1\s*(#[0-9a-fA-F]{3,8})\1/g,'$1$2$3$1')
  .replace(/\b(const|let|var|function|return|typeof|throw|new|case|default|import|export|switch|while|for)([A-Z_][A-Za-z0-9_]*)\b/g,(match,keyword,suffix)=>keyword==='for'&&suffix==='Each'?match:keyword+' '+suffix)
  .replace(/\bconst\.([A-Za-z_$][\w$]*)(\s*=)/g,'const $1$2')
  .replace(/\brequire\(['\"]remotion['\"]\)\.useCurrentFrame\(\)/g,'useCurrentFrame()')
 ...
 // CPU Performance: strip expensive effects that the GLM might still generate
 normalized=normalized.replace(/backdropFilter\s*:\s*['"][^'"]*['"]\s*,?/g,'');
 normalized=normalized.replace(/mixBlendMode\s*:\s*['"][^'"]*['"]\s*,?/g,'');
 normalized=normalized.replace(/<feTurbulence[^/]*\/>/g,'');
 normalized=normalized.replace(/<feGaussianBlur[^/]*\/>/g,'');
 normalized=normalized.replace(/<feColorMatrix[^/]*\/>/g,'');
 normalized=normalized.replace(/<filter[^]*?<\/filter>/g,'');
if(scene?.map||Number.isInteger(scene?.backgroundIndex)){
 ...
 normalized=normalized.replace(/<(OffthreadVideo|Video)\b([^>]*?)\bendAt=(?:\{[^}]+\}|"[^"]*"|'[^']*')([^>]*?)>/g,'<$1$2$3>');
...
const isSafeBoundedFor=(node,numericConstants)=>{
 ...
 const bound=node.condition.right;
 const constantValue=part=>ts.isNumericLiteral(part)?Number(part.text):ts.isIdentifier(part)?numericConstants.get(part.text):NaN;
 const limit=bound&&ts.isBinaryExpression(bound)&&[ts.SyntaxKind.MinusToken,ts.SyntaxKind.PlusToken].includes(bound.operatorToken.kind)
  ?(bound.operatorToken.kind===ts.SyntaxKind.MinusToken?constantValue(bound.left)-constantValue(bound.right):constantValue(bound.left)+constantValue(bound.right))
  :bound?constantValue(bound):NaN;
...
export function validateMotionCode(code,scene=null){
 ...
 let frame=false,exported=false,importsD3=false,firstAbsoluteFill=null,videoElementCount=0;const importedRemotion=new Set(),mediaTags=new Set();
 function visit(node){
  ...
  if((ts.isJsxOpeningElement(node)||ts.isJsxSelfClosingElement(node))&&ts.isIdentifier(node.tagName)){
   ...
   if(['Video','OffthreadVideo'].includes(node.tagName.text))videoElementCount++;
 ...
 visit(ast);if(scene?.asset?.kind==='image'&&!frame)throw Error('Fotografias precisam de movimento ou efeito ligado a useCurrentFrame.');if(!exported)throw Error('Exporte um componente React default. Cenas estáticas intencionais são permitidas.');
 if(videoElementCount>1)throw Error('Use apenas um componente de vídeo por cena; não repita o mesmo clipe em tela cheia e em quadro menor.');
 for(const tag of mediaTags)if(!importedRemotion.has(tag))throw Error(`O componente ${tag} foi usado sem import do Remotion.`);
 if(scene?.asset?.kind==='video'&&!mediaTags.has('Video')&&!mediaTags.has('OffthreadVideo'))throw Error('A cena recebeu vídeo real, mas o código não o mostra. A explicação deve aparecer sobre o vídeo.');
 if(scene?.asset?.kind==='image'&&!mediaTags.has('Img'))throw Error('A cena recebeu fotografia, mas o código não a mostra.');
 if(scene&&/\bCHAPTER\s+(?:ONE|TWO|THREE|\d+)\b/i.test(code)&&!/\bchapter\b/i.test(String(scene.narration||'')))throw Error('Marcador de capítulo não narrado não pode cobrir a cena.');
 if(scene?.countries?.length){
  const narration=String(scene.narration||'').toLowerCase();
  const emphasizedCode=code.replace(/\{\/\*[\s\S]*?\*\/\}/g,'').replace(/<([A-Za-z][\w.]*)\b[^>]*>\s*(?:Footage|Video|Source|Credit)\s*:[^<]*<\/\1>/gi,'');
  for(const country of scene.countries){
   const name=String(country||'').trim();
   if(!name||narration.includes(name.toLowerCase()))continue;
   const literal=new RegExp('\\b'+name.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')+'\\b','i');
   if(literal.test(emphasizedCode))throw Error('Nome de país ainda não narrado não pode aparecer em destaque: '+name);
  }
 }
 ...
 if(/mixBlendMode\s*:/i.test(code))throw Error('Evite mixBlendMode para não estourar a imagem e manter renderização rápida. Use cores sólidas com opacidade.');
 if(/backdropFilter\s*:/i.test(code))throw Error('backdropFilter is forbidden for CPU rendering performance. Use solid rgba() backgrounds instead.');
 if(/<fe(?:Turbulence|GaussianBlur|ColorMatrix|Composite|Blend)\b/i.test(code))throw Error('SVG filter primitives are forbidden for CPU rendering performance.');
 if(/<filter[\s>]/i.test(code))throw Error('SVG filter elements are forbidden for CPU rendering performance.');
...
For the long documentary, treat the FIRST THREE scenes as a connected visual hook: show the subject, sharpen the question and establish why it matters, with the strongest researched image or fact early. Give the LAST scene a visual payoff or conclusion that feels intentional with the narration and leaves a settled image before a brief fade to black; these are bookends, not reusable opening/closing templates. A title may appear when it strengthens the hook, but do not require a logo, generic title card or generic end screen. Direct meaning, hierarchy, narrative development and breathing room. Do not write TSX. Do not dictate transitions, coordinates, colors, effect names or frame-by-frame choreography. Available media and maps are an inventory, not mandatory layouts. Distinguish mustShow obligations from optional opportunities. Preserve the motion designer's freedom to invent. Exact, relevant footage without a new narrated fact should play cleanly. Contextual footage must never masquerade as evidence of the named subject: use it as an atmospheric base while a researched explanatory graphic carries the actual meaning. Invisible mechanisms need a legible Remotion explanation showing relationships, direction and causality over real footage whenever available. Standalone graphics are rare exceptions. Maps, photos, charts, type and graphic layers may be combined when meaningful. Avoid repetitive fades, repeated layouts, presentation slides, excessive text, decorative clutter and visual fatigue. A cut is allowed whenever continuity would weaken the story. Use only facts and places supplied in the timeline. A place from research or neighboring scenes may guide imagery, but only names spoken in the current scene may become on-screen emphasized text.`;
...
VISUAL BREATHING ROOM & NO TEXT CLUTTER: Exactly one focal message may dominate at a time. Do not stack a title, chapter, location boxes, question and caption in the same beat. On-screen emphasized place names, numbers and facts must be spoken in CURRENT_SCENE.narration; research and neighboring scenes are context, not permission to display an unsaid label. For a new fact or mechanism over selected footage, animate the explanation while the real video remains clearly visible; avoid an opaque black or dark full-frame panel. Do NOT put cards, text boxes, or graphics on every single scene! Descriptive VIDEO scenes should play unobstructed, with no text or a minimal location identifier when useful. Narrated data and explanations take priority over any suggested proportion of clean scenes. Let descriptive scenes breathe after graphics; keep a necessary explanation even when the previous scene also explained data. Never bombard the viewer with non-stop text card after text card.
...
MOTION GRAPHICS & DATA VISUALIZATION: Explain the current narrated idea, not just decorate footage. When it introduces a verified quantity, geographic mechanism or comparison, animate the relationship at the spoken moment: scale, flow, layering, route or measured difference. Use one focal graphic per scene and verified labels only. A scene with kind diagram MUST visibly animate a causal schematic in Remotion, with at least two meaningfully distinct elements and a directional or comparative change; no generic title card, solitary stock ocean or zoom-plus-caption substitute. If a country outline or border is needed, use the supplied accurate ContextMap rather than invented geometry. If the asset is contextual, let the graphic carry the explanation and do not imply that the media depicts the exact place. Reserve clean footage for exact relevant footage without a new fact.
CINEMATIC FOOTAGE & B-ROLL: When the scene is atmospheric scenery or showing a location, let the footage breathe with smooth camera motion (subtle scale 1.04 to 1.10, gentle drift) and completely uncluttered framing. Always keep video footage muted (<OffthreadVideo muted ... />). Use only ONE video element per scene; never repeat the same clip as both background and inset.
PHOTOS & ARCHIVAL IMAGES: Every photograph needs visible, restrained motion or a purposeful effect. Choose freely: subject-aware push, pan, reveal, detail highlight, mask or comparison. A generic zoom plus caption is insufficient. Reveal a meaningful part of the photograph, pan across the subject, animate a crop or mask, highlight a visible detail, or build a researched annotation tied to the spoken line. Avoid unnecessary text. When narration provides data, age, dates or dimensions, animate that researched fact over the photo. Never leave a photograph static. Preserve image proportions and detail; do not fake documents or provenance. Avoid bright full-frame exposure flashes, white wash, blend modes that destroy detail and opacity above 1.
MAP SCENES: The parent mounts a Natural Earth country map and an optional explicitly requested schematic connection. Country names appear only when spoken in this scene. A centroid connection is NOT a measured travel path; never claim that it follows a strait, road or shipping route. DO NOT attempt to draw custom bezier route curves, arrows, or country pins in SceneX.tsx. Keep the map as the focus. Add at most one purposeful text or measurement overlay at a time, using words and numbers spoken in this scene. Do not add chapter cards, country boxes or a second headline while the map already explains the narration.
...
DOCUMENTARY BOOKENDS: currentScene.editorialRole is opening, openingDevelopment, body or closing. If opening, establish a visibly distinct first beat within the first second: show the specific researched subject with one clear spatial, comparative or kinetic question the next scenes will answer. Give the real footage a purposeful visual development in the first 2-3 seconds: for example, reveal the spoken contrast through scale, movement, framing or one short headline that the viewer can read immediately. A small side metric over an otherwise unchanged wide shot does not provide a strong opening. Keep the focal element large enough to be understood on a phone, then hand off to the explanation. Vary the solution by topic; do not reuse a fixed intro template. Do not pile a title, chapter marker and several labels into the same opening beat. The viewer must recognize a purposeful opening even with audio muted; a small corner label or ordinary lower-third over footage is insufficient. Establish one strong central or center-left visual focus immediately, using the researched subject itself, a legible short hook, or both. If there is a title, make it readable at normal playback within the first second (at least 64px), then reveal one researched visual contradiction before the cut. Keep the real footage or photograph visible and do not stack competing titles. If openingDevelopment, continue the first scene's unresolved question through a distinct, researched visual: show the stakes, a revealing comparison or the geographic mechanism, without repeating the same title or card. You may use one large, legible title or place name when it adds meaning, but never force a reusable title-card template, logo or black screen. Let the composition develop during the existing scene duration and hand off clearly to scene two. If closing, make the answer visually unmistakable even with audio muted: transform or resolve the opening motif using the final researched image, diagram or verified fact, then hold a deliberate final beat for at least the final 1.5 seconds during the existing narration/audio tail. Favor a focused central visual or short closing statement, then let the parent composition handle a brief final fade to black. Do not end on an unmarked stock photograph with only a small caption. Do not finish mid-motion, add a generic subscribe card, or claim the story is resolved when the facts do not support it. These instructions apply only to the long documentary; vertical Shorts start immediately without ceremonial bookends.
...
FACTS: Use only supplied narration/research, exact geographic data and verified chart values. Do not invent numbers, coordinates, borders, images or travel routes. Do not portray an unrelated location as the requested place. An asset marked representationRole contextual is thematic B-roll, not proof of the named location, person, structure or event: use it for broad atmosphere or concepts and never attach a label claiming it is the exact subject. An asset marked representationRole illustrative is AI-generated conceptual art, never documentary evidence or an exact depiction; animate it as a still, and never label it as authentic footage, an archival photograph or the exact named subject. Unsupported quantitative graphics must become qualitative explanation.
TOOLS: For media use staticFile(scene.asset.src); trimStart is seconds and must be converted to frames for startFrom (e.g. startFrom={Math.round((scene.asset.trimStart||0)*fps)}). NEVER use endAt on OffthreadVideo or Video — the parent Sequence manages scene duration automatically, and endAt will prematurely kill the footage. Use objectFit cover for both photos and footage. Named imports from remotion: AbsoluteFill, Sequence, Series, Img, OffthreadVideo, Video, Freeze, interpolate, interpolateColors, spring, Easing, useCurrentFrame, useVideoConfig, staticFile, random.
...
CREATIVE FREEDOM OVERRIDE: Earlier choreography, percentage, word-count and continuous-motion suggestions are defaults, not hard creative requirements. Descriptive VIDEO may play cleanly without an added effect or camera drift. PHOTOGRAPHS ALWAYS need visible restrained movement or an authored effect; never leave an image static. When the narration presents a number, date, age, dimension, comparison or important explanatory fact, show a meaningful animation tied to that precise spoken fact over the video or image. Never use decorative quotas to remove a needed explanation. Respect motionStyle hold only for descriptive video; a static React overlay is permitted there. Necessary explanations may span consecutive scenes when editorialReason justifies it. Preserve facts, legibility, provided map geometry and technical safety.\nPERFORMANCE (CPU RENDERER — MANDATORY): This video is rendered on a CPU-only server without GPU acceleration. The following CSS and SVG features are ABSOLUTELY FORBIDDEN because they make rendering impossibly slow: (1) NEVER use backdropFilter on any element. (2) NEVER use the CSS filter property with blur() on any element larger than 200px. (3) NEVER use mixBlendMode on any element — use opacity and solid rgba colors instead. (4) NEVER use SVG filters: no <filter>, no <feTurbulence>, no <feGaussianBlur>, no <feColorMatrix>. (5) box-shadow blur radius MUST be at most 6px — use solid rgba() borders or backgrounds for vignettes. (6) text-shadow blur radius MUST be at most 4px. (7) NEVER stack more than 2 CSS filter functions on one element. Use transform (translate, scale, rotate), opacity, and solid colors for all visual effects. These create zero per-pixel computation cost.\nEXECUTION: Deterministic frame-driven React only. No network, filesystem, external URLs, runtime packages, dynamic imports, browser globals, side effects, eval or timers. Arrays/map are preferred. A numeric for loop is allowed only to build deterministic visual arrays when it starts from a numeric literal, uses a numeric literal upper bound no greater than 500 and increments with ++. No while/do loops. No HTML video/img. Do not registerRoot or Composition. Keep the code focused on this one scene and reasonably compact. Code must be valid TSX.`;

const motionTimeout=()=>Math.min(420000,Math.max(60000,Number(process.env.ATLAS_MOTION_TIMEOUT_MS)||210000));
...
export function sceneUnits(manifest){
 ...
 for(const scene of units){
  if(scene.asset||scene.map||scene.kind==='diagram')continue;
...
function compactScene(scene){
 return {id:scene.id,index:scene.index,editorialRole:scene.editorialRole,start:scene.originalStart,end:scene.originalEnd,narration:scene.narration,heading:scene.heading,caption:scene.caption,kind:scene.kind,treatment:scene.treatment,label:scene.label,labelStyle:scene.labelStyle,labelRole:scene.labelRole,location:scene.location,diagramIntent:scene.diagramIntent,countries:scene.countries,routes:scene.routes,routeFrom:scene.routeFrom,routeTo:scene.routeTo,routeEvidence:scene.routeEvidence,mapIntent:scene.mapIntent,motionStyle:scene.motionStyle,editorialReason:scene.editorialReason,explanationEvidence:scene.explanationEvidence,layout:scene.layout,hasAsset:Boolean(scene.asset),asset:scene.asset?{src:scene.asset.src,kind:scene.asset.kind,trimStart:scene.asset.trimStart,duration:scene.asset.duration,representationRole:scene.asset.representationRole}:undefined,backgroundIndex:scene.backgroundIndex,chart:scene.chart?{unit:scene.chart.unit,values:scene.chart.values,source:scene.chart.source}:undefined};
}
...
export function normalizeMotionResult(result,seconds,fps=30){
 ...
 try{validateBeats(result.beats,seconds);}catch{result.beats=[{start:0,end:seconds,action:String(result.intent||'Purposeful visual development continues throughout the scene.')}];}
...
const readLatestValidAttempt=async(folder,scene)=>{
 for(let attempt=5;attempt>=1;attempt--){
...
export function normalizeVisualBible(bible,scenes){
 ...
 const sceneDirectives=scenes.map(scene=>{
  ...
  const spokenCountryNames=new Set((scene.countries||[]).filter(name=>String(scene.narration||'').toLowerCase().includes(String(name).toLowerCase())).map(name=>String(name).toLowerCase()));
  const permittedMustShow=items=>items.map(String).filter(item=>!(scene.countries||[]).some(name=>String(name).toLowerCase()===item.toLowerCase())||spokenCountryNames.has(item.toLowerCase()));
  if(existing&&typeof existing==='object'){
   return {
    id:scene.id,
    intent:String(existing.intent||scene.heading||'Visual explanation of narrative segment'),
    mustShow:permittedMustShow(Array.isArray(existing.mustShow)?existing.mustShow:(scene.countries||[])),
  ...
  return {
   ...
   mustShow:permittedMustShow(scene.countries||[]),
   opportunities:[],
...
export function generateCleanMediaSceneCode(scene){
 ...
 const vertical=Boolean(scene.isShort);
 const fontSize=vertical?Math.max(52,roleSizes[scene.labelRole]||52):(roleSizes[scene.labelRole]||36);
 ...
 const shortPlacements=[
  "left:80,top:260,padding:'18px 22px',borderRadius:8,background:'rgba(7,20,18,.68)',borderLeft:'5px solid "+tone.accent+"'",
  "left:80,bottom:310,padding:'14px 22px',background:'rgba(7,20,18,.62)',borderLeft:'5px solid "+tone.accent+"'",
  "left:'50%',top:360,transform:'translateX(-50%)',padding:'14px 26px',background:'rgba(5,14,18,.48)',borderBottom:'3px solid "+tone.accent+"',textAlign:'center'",
  "right:70,bottom:380,padding:'14px 22px',borderRadius:12,background:'rgba(7,20,18,.68)',border:'1px solid "+tone.accent+"'"
 ];
 const labelStyle=(vertical?shortPlacements:placements)[variant]+','+tone.style;
...
export default function MotionScene({scene}: {scene:any; context?:any}) {
 ...
 const scale=scene.asset.kind==='video'?1:1.025+progress*.045;
 const drift=scene.asset.kind==='video'?0:(progress-.5)*18;
...
export function generateFallbackSceneCode(scene,isShort=false){
 if(scene?.kind==='diagram'){
  const diagram=diagramFallbackCode(scene);
  if(diagram)return diagram;
  throw Error('Diagrama sem alternativa factual segura. Retome a cena para obter uma explicação específica; não substituir por conteúdo de outro assunto.');
 }
 const dur=scene?.durationInFrames||90;
 const isVideo=scene?.asset?.kind==='video';
 const hasAsset=Boolean(scene?.asset?.src);
 const assetSrc=scene?.asset?.src||'';
 const heading=narrationNeedsExplanation(scene)&&scene.asset?factualCaption(scene):(scene?.heading||'');
 const caption=scene?.caption?scene.caption.replace(/"/g,'\\"'):'';
 if(hasAsset&&!isVideo){
  const photoLabel=narrationNeedsExplanation(scene)?String(factualCaption(scene)).split(/\s+/).slice(0,8).join(' '):'';
  return `import React from 'react';
import {AbsoluteFill,Img,interpolate,staticFile,useCurrentFrame} from 'remotion';
export default function MotionScene({scene}: {scene:any; context?:any}){
 const frame=useCurrentFrame();
 const duration=Math.max(1,scene.durationInFrames||${dur});
 const reveal=interpolate(frame,[0,Math.max(1,Math.round(duration*.72))],[0,100],{extrapolateLeft:'clamp',extrapolateRight:'clamp'});
 const pan=interpolate(frame,[0,duration],[2,-2],{extrapolateLeft:'clamp',extrapolateRight:'clamp'});
 const photo={width:'100%',height:'100%',objectFit:'cover' as const,transform:'translateX('+pan+'%) scale(1.06)'};
 return <AbsoluteFill style={{overflow:'hidden',background:'#101817'}}>
  <Img src={staticFile(${JSON.stringify(assetSrc)})} style={{...photo,opacity:.42}}/>
  <Img src={staticFile(${JSON.stringify(assetSrc)})} style={{...photo,position:'absolute',clipPath:'inset(0 '+(100-reveal)+'% 0 0)'}}/>
  {${JSON.stringify(photoLabel)} && <div style={{position:'absolute',left:80,bottom:80,maxWidth:600,padding:'14px 22px',borderRadius:8,background:'rgba(7,16,25,.8)',color:'#f2ecdd',fontSize:42,fontWeight:700,opacity:interpolate(frame,[0,Math.max(1,Math.min(18,duration-1))],[0,1],{extrapolateRight:'clamp'})}}>{${JSON.stringify(photoLabel)}}</div>}
 </AbsoluteFill>;
}`;
 }
  ...
  return (
    <AbsoluteFill style={{backgroundColor: 'transparent', overflow: 'hidden', fontFamily: "'Helvetica Neue', Arial, sans-serif"}}>
      ...
      <AbsoluteFill style={{justifyContent: 'flex-end', padding: ${isShort ? "'0 40px 260px'" : 80}}}>
        <div style={{background: 'rgba(7, 16, 25, 0.78)', padding: '12px 24px', borderRadius: 8, display: 'inline-block', maxWidth: ${isShort ? "'90%'" : "'620px'"}}}>
          <h2 style={{margin: 0, color: '#F2ECDD', fontSize: ${isShort ? 46 : 38}, fontWeight: 700, letterSpacing: 1, opacity:interpolate(frame,[0,Math.max(1,Math.min(18,dur-1))],[0,1],{extrapolateLeft:'clamp',extrapolateRight:'clamp'})}}>{${JSON.stringify(heading)}}</h2>
...
export function withDocumentaryBookends(scenes){return scenes.map((scene,index)=>({...scene,editorialRole:index===0?'opening':index===scenes.length-1?'closing':index<3?'openingDevelopment':'body'}));}
export function usesCleanMediaFastPath(scene,repairThisScene=false,{version='v2'}={}){return scene.editorialRole==='body'&&!(version==='v3'&&Number(scene.originalStart||0)<30)&&scene.asset?.kind==='video'&&['clean','label'].includes(scene.treatment)&&!narrationNeedsExplanation(scene)&&!repairThisScene;}
...
export async function authorMotion(s,j,manifest,{root,log,repairError,repairSceneIndexes=[]}){
 ...
 const scenes=withDocumentaryBookends(sceneUnits(manifest)),timeline=scenes.map(compactScene),model=j.productionVersion==='v3'?'gpt-6-luna':(s.motionModel||'glm-5.3-flash');
 ...
 if(!visualBible){
  log(j,`Modelo (${model}): criando a direção visual global para ${scenes.length} cenas.`);
  try{
   ...
   const rawBible=await generateJSON({...s,sceneProvider:'go',goModel:model,reasoningEffort:model==='glm-5.3-flash'?'low':undefined,timeoutMs:motionTimeout()},j,directorContract+'\nVIDEO_DATA: '+JSON.stringify({title:j.title,sceneCount:scenes.length,timeline:briefTimeline})+'\nOnly direct the sampled scene IDs. Other scene directives will be derived from the full timeline locally.');
   visualBible=normalizeVisualBible(rawBible,scenes);
  }catch(err){
   log(j,`Modelo: direção global indisponível (${String(err.message).slice(0,160)}). Usando direção segura derivada das cenas já revisadas.`);
 ...
 let previousHandoff=null;const handoffs=[];let fallbackCount=0;let consecutiveFallbacks=0;const maxFallbacks=j.productionVersion==='v3'?Math.max(1,Math.ceil(scenes.length*.05)):Math.max(2,Math.ceil(scenes.length*.05));const maxConsecutiveFallbacks=3;
 for(const scene of scenes){
  ...
  if(saved?.fallback)log(j,`Cena ${scene.index+1}: reserva visual já validada preservada na retomada.`);
  ...
  if(usesCleanMediaFastPath(scene,repairThisScene,{version:j.productionVersion||'v2'})){
   ...
   consecutiveFallbacks=0;log(j,`Cena ${scene.index+1}/${scenes.length}: mídia ${scene.treatment==='label'?'com identificação mínima':'limpa'}, sem chamada adicional ao modelo.`);
   continue;
  }
  log(j,`${model} programando cena ${scene.index+1} de ${scenes.length}...`);
  let accepted=null;
  const attemptLimit=j.productionVersion==='v3'?5:3;
  for(let attempt=0;attempt<attemptLimit;attempt++){
   let result=saved;
   if(!result){
    if(attempt>0)log(j,`${model}: corrigindo a cena ${scene.index+1}/${scenes.length} (${attempt+1}/${attemptLimit})...`);
    const attemptStarted=Date.now();
    const chosenModel=j.productionVersion==='v3'&&attempt>=3?'glm-5.3-flash':motionModelForAttempt(model,attempt);
    try{
     const assignment={artDirection:directive,currentScene:promptScene,context,feedback,previousCode};
     const prompt=(attempt===0||j.productionVersion==='v3'&&attempt===3?sceneContract:rescueContract)+'\nSCENE_ASSIGNMENT: '+JSON.stringify(attempt===0||j.productionVersion==='v3'&&attempt===3?assignment:compactRescueAssignment(assignment))+(j.productionVersion==='v3'?'\nEDITORIAL IDENTITY: Curious, observant and occasionally dry; let a researched contradiction guide visual wit, never force a joke or weaken documentary accuracy.':'');
     result=await generateJSON({...s,sceneProvider:'go',goModel:chosenModel,reasoningEffort:chosenModel==='glm-5.3-flash'?'low':undefined,timeoutMs:chosenModel==='gpt-6-luna'?motionTimeout():attempt===0?motionTimeout():120000},j,prompt);
     await writeFile(path.join(folder,`scene-${scene.index}-attempt-${attempt+1}.json`),JSON.stringify(result,null,2));
    }catch(err){
     const reason=/Tempo limite|sem resposta/i.test(err.message)?'timeout':/inferência|inference_failed/i.test(err.message)?'inference-failed':/fora do formato|JSON/i.test(err.message)?'invalid-json':'provider-error';
     await writeFile(path.join(folder,'scene-'+scene.index+'-failure-'+(attempt+1)+'.json'),JSON.stringify({reason,model:j.productionVersion==='v3'&&attempt>=3?'glm-5.3-flash':motionModelForAttempt(model,attempt),seconds:Math.round((Date.now()-attemptStarted)/1000)}));
     log(j,'Cena '+(scene.index+1)+': '+reason+' em '+chosenModel+'; tentativa '+(attempt+1)+'/'+attemptLimit+'. '+String(err.message).slice(0,145));
     if(attempt===attemptLimit-1)break;
   ...
   try{
    ...
    const reason=!result?.code?'missing-code':!result?.handoff?'missing-handoff':/TSX inválido/i.test(e.message)?'invalid-tsx':'validation-error';
    await writeFile(path.join(folder,`scene-${scene.index}-failure-${attempt+1}.json`),JSON.stringify({reason,model:j.productionVersion==='v3'&&attempt>=3?'glm-5.3-flash':motionModelForAttempt(model,attempt),detail:String(e.message).slice(0,250)}));
    const explanation={'missing-code':'faltou o código da animação','missing-handoff':'faltou a transição entre cenas','invalid-tsx':'o código da animação veio inválido','validation-error':'a resposta não passou na validação'}[reason];
    log(j,`Cena ${scene.index+1}: a API respondeu, mas ${explanation}; tentativa ${attempt+1}/${attemptLimit}.`);
  ...
  if(!accepted){
   if(fallbackCount>=maxFallbacks||consecutiveFallbacks>=maxConsecutiveFallbacks)throw new Error(`Falha visual acima do limite seguro: ${fallbackCount}/${scenes.length} cenas de reserva (máximo ${maxFallbacks}) ou ${maxConsecutiveFallbacks} consecutivas. Produção pausada; cenas anteriores preservadas para retomada.`);
   const code=generateFallbackSceneCode(scene);
   validateMotionCode(code,scene);
   accepted={intent:'Animated reserve for isolated model failure',beats:[{start:0,end:scene.durationInFrames/scene.fps,action:'Safe visual treatment over the selected media'}],handoff:{opening:'editorial cut',persistentElements:[],endState:'media holds',motionVector:'animated reveal',nextOpportunity:'resume authored visuals'},code,fallback:true};
   await writeFile(filename,JSON.stringify(accepted,null,2));await writeFile(path.join(folder,`Scene${scene.index}.tsx`),code);
   log(j,`Cena ${scene.index+1}/${scenes.length}: alternativa animada usada após falha isolada do modelo.`);
  }
  if(accepted.fallback){fallbackCount++;consecutiveFallbacks++;j.auto.visualFallbacks=[...new Set([...(j.auto.visualFallbacks||[]),`shot-${scene.index+1}`])];}else{consecutiveFallbacks=0;j.auto.visualFallbacks=(j.auto.visualFallbacks||[]).filter(id=>id!==`shot-${scene.index+1}`&&id!==scene.id);}
  previousHandoff=accepted.handoff;handoffs.push(previousHandoff);
  j.auto.motion.completed=scene.index+1;j.auto.progress=70+Math.round((j.auto.motion.completed/scenes.length)*15);
  log(j,accepted.fallback?`Cena ${scene.index+1}/${scenes.length}: reserva visual pronta.`:`${model} programou cena ${scene.index+1}/${scenes.length} com sucesso.`);
 }
 ...
 const children=scenes.map(scene=>{const needsBackdrop=Boolean(scene.map||!scene.asset);return `<Sequence from={${scene.from}} durationInFrames={${scene.durationInFrames}}><AbsoluteFill>${needsBackdrop?`<SceneBackdrop scene={scenes[${scene.index}]} backgroundScene={Number.isInteger(scenes[${scene.index}].backgroundIndex)?scenes[scenes[${scene.index}].backgroundIndex]:null} duration={${scene.durationInFrames}}/>`:''}<Scene${scene.index} scene={scenes[${scene.index}]} context={contexts[${scene.index}]} /></AbsoluteFill></Sequence>`;}).join('');
    let bgmTag='';
    const jobBgmRel=`auto/${j.auto.runId}/bgm.mp3`;
    const balancedMusic=j.productionVersion==='v3'&&Boolean(j.selectedMusic?.normalization);
    const vol=balancedMusic?0.075:(j.productionVersion==='v3'?0.045:(j.selectedMusic?.defaultVolume||0.06));
    try{
     ...
     log(j,`BGM: trilha "${j.selectedMusic?.name||'selecionada'}" com ${j.productionVersion==='v3'?'abertura breve e redução progressiva sob a voz':'ducking dinâmico'} (base ${Math.round(vol*100)}%).`);
    }catch{
     try{
  ...
  const entry=`import React from 'react';import {AbsoluteFill,Audio,Loop,Sequence,Composition,registerRoot,staticFile,useCurrentFrame} from 'remotion';import {SceneBackdrop} from './motion-kit';\n${imports}\nconst scenes=${JSON.stringify(scenes)};\nconst contexts=${JSON.stringify(contexts)};\nconst totalFrames=${Math.ceil(manifest.duration*30)};const bgmVol=(f)=>{const tail=Math.max(0,Math.min(1,(totalFrames-f)/22));if(${j.productionVersion==='v3'}){const opening=Math.max(0,1-f/24);const sc=scenes.find(s=>f>=s.from&&f<s.from+s.durationInFrames);const base=sc?.kind==='chart'?${balancedMusic?0.05:0.03}:${vol};return (base+${balancedMusic?0.035:0.02}*opening)*tail;}if(f<75)return 0.18*tail;const sc=scenes.find(s=>f>=s.from&&f<s.from+s.durationInFrames);if(sc?.kind==='chart')return 0.03*tail;return ${vol}*tail;};\nconst Video=()=>{const frame=useCurrentFrame();const fade=Math.max(0,Math.min(1,(frame-(totalFrames-22))/21));return <AbsoluteFill style={{background:'#142d32',fontFamily:'Arial, sans-serif'}}><Audio src={staticFile(${JSON.stringify(manifest.voice)})}/>${bgmTag}${children}<AbsoluteFill style={{background:'#000',opacity:fade}}/></AbsoluteFill>;};registerRoot(()=> <Composition id="AutomaticVideo" component={Video} durationInFrames={${Math.ceil(manifest.duration*30)}} fps={30} width={1920} height={1080}/>);`;
...
- The parent supplies the same accurate Natural Earth basemap as long videos, adapted to 1080x1920. Keep the root transparent for scene.map. Do not invent polygons or paths. Overlay typography and highlights freely without hiding the supplied geography.
- NEVER draw a small circular globe or tiny horizontal map centered with empty space!
- The supplied map fits the named countries in the portrait safe area. A country-level dataset cannot supply a true close-up of a narrow waterway. Use real media for local detail; do not invent a glowing corridor.
- Do not replace provided map geometry with an invented map or an unverified pin.
VERTICAL FOOTAGE:
- Use objectFit cover. Scale footage to fill 1080x1920 without letterboxing. Always keep video footage muted (<OffthreadVideo muted ... />). Use only ONE video element per scene; never repeat the same clip as both background and inset.
...
TOOLS:
- staticFile(scene.asset.src) for media. trimStart is in seconds and must be converted to frames for startFrom. NEVER use endAt on OffthreadVideo. Named imports from remotion: AbsoluteFill, Sequence, Series, Img, OffthreadVideo, Video, Freeze, interpolate, interpolateColors, spring, Easing, useCurrentFrame, useVideoConfig, staticFile, random.
CREATIVE FREEDOM OVERRIDE: Earlier choreography, percentage, word-count and continuous-motion suggestions are defaults, not hard creative requirements. Descriptive VIDEO may play cleanly without an added effect or camera drift. PHOTOGRAPHS ALWAYS need visible restrained movement or an authored effect; never leave an image static. When the narration presents a number, date, age, dimension, comparison or important explanatory fact, show a meaningful animation tied to that precise spoken fact over the video or image. Never use decorative quotas to remove a needed explanation. Respect motionStyle hold only for descriptive video; a static React overlay is permitted there. Necessary explanations may span consecutive scenes when editorialReason justifies it. Preserve facts, legibility, provided map geometry and technical safety.\nEXECUTION:
...
export async function authorShortsMotion(s,j,manifest,{root,log,shortIndex}){
 ...
 const scenes=sceneUnits(manifest),timeline=scenes.map(compactScene),model=j.productionVersion==='v3'?'gpt-6-luna':(s.motionModel||'glm-5.3-flash');
 scenes.forEach(sc => { sc.isShort = true; });
 log(j,`Short ${shortIndex+1}: ${model} programando ${scenes.length} cenas verticais.`);
 ...
 if(!visualBible){
  for(let attempt=0;attempt<3;attempt++){
   try{
    const rawBible=await generateJSON({...s,sceneProvider:'go',goModel:model,reasoningEffort:model==='glm-5.3-flash'?'low':undefined,timeoutMs:240000},j,shortsDirectorContract+'\nVIDEO_DATA: '+JSON.stringify({title:manifest.title,timeline}));
    visualBible=normalizeVisualBible(rawBible,scenes);
 ...
 let previousHandoff=null;const handoffs=[];let fallbackCount=0;const maxFallbacks=Math.max(1,Math.ceil(scenes.length*.05));
 for(const scene of scenes){
  ...
  const context={visualBible,timeline,previousScene:timeline[scene.index-1]||null,currentScene:timeline[scene.index],nextScene:timeline[scene.index+1]||null,previousHandoff};
  const promptScene=cleanPromptScene(scene);
  log(j,`Short ${shortIndex+1}: ${model} programando cena ${scene.index+1}/${scenes.length}...`);
  let accepted=null;
  if(!saved){
   for(let previousAttempt=1;previousAttempt<=(j.productionVersion==='v3'?5:3);previousAttempt++){
    const candidate=await readJSON(path.join(folder,`scene-${scene.index}-attempt-${previousAttempt}.json`));
    if(!candidate)continue;
    try{
     normalizeMotionResult(candidate,scene.durationInFrames/scene.fps,scene.fps);candidate.code=normalizeMotionCode(candidate.code,scene);validateBeats(candidate.beats,scene.durationInFrames/scene.fps);validateHandoff(candidate.handoff);validateMotionCode(candidate.code,scene);
     saved=candidate;log(j,`Short ${shortIndex+1}: cena ${scene.index+1} recuperada de tentativa anterior já salva.`);break;
    }catch{}
   }
  }
  if(j.productionVersion==='v3'&&scene.index>0&&scene.index<scenes.length-1&&scene.asset?.kind==='video'&&['clean','label'].includes(scene.treatment)&&!narrationNeedsExplanation(scene)){
   const code=generateCleanMediaSceneCode(scene);validateMotionCode(code,scene);
   accepted={intent:'Documentary footage',beats:[{start:0,end:scene.durationInFrames/scene.fps,action:'Footage remains clear'}],handoff:{opening:'direct cut',persistentElements:[],endState:'media visible',motionVector:'deliberate cut',nextOpportunity:'next fact'},code};
   await writeFile(filename,JSON.stringify(accepted,null,2));await writeFile(path.join(folder,`Scene${scene.index}.tsx`),code);
   previousHandoff=accepted.handoff;handoffs.push(previousHandoff);
   log(j,'Short '+(shortIndex+1)+', cena '+(scene.index+1)+': vídeo simples com componente local.');
   continue;
  }
  const attemptLimit=j.productionVersion==='v3'?5:3;
  for(let attempt=0;attempt<attemptLimit;attempt++){
   let result=saved;
   if(!result){
    if(attempt>0)log(j,`Short ${shortIndex+1}: corrigindo cena ${scene.index+1} (${attempt+1}/${attemptLimit})...`);
    try{
     const assignment={artDirection:directive,currentScene:promptScene,context,feedback};
     const chosenModel=j.productionVersion==='v3'&&attempt>=3?'glm-5.3-flash':motionModelForAttempt(model,attempt);
     result=await generateJSON({...s,sceneProvider:'go',goModel:chosenModel,reasoningEffort:chosenModel==='glm-5.3-flash'?'low':undefined,timeoutMs:chosenModel==='gpt-6-luna'?motionTimeout():120000},j,(attempt===0||j.productionVersion==='v3'&&attempt===3?shortsSceneContract:rescueContract)+'\nSCENE_ASSIGNMENT: '+JSON.stringify(attempt===0||j.productionVersion==='v3'&&attempt===3?assignment:compactRescueAssignment(assignment)));
     await writeFile(path.join(folder,`scene-${scene.index}-attempt-${attempt+1}.json`),JSON.stringify(result,null,2));
    }catch(err){
     if(attempt===attemptLimit-1){if(j.productionVersion==='v3')break;throw new Error('Short '+(shortIndex+1)+', cena '+(scene.index+1)+': modelo de motion indisponível após três tentativas. Retome sem perder as cenas prontas.');}
   ...
   try{
    ...
    saved=null;feedback=`Validation failed: ${e.message} Fix the defect.`;log(j,`Short ${shortIndex+1}, cena ${scene.index+1}: tentativa ${attempt+1}/${attemptLimit} rejeitada: ${e.message}`);
    if(attempt===attemptLimit-1){if(j.productionVersion==='v3')break;throw new Error('Short '+(shortIndex+1)+', cena '+(scene.index+1)+': código visual inválido após três tentativas. Retome sem perder as cenas prontas.');}
   }
  }
  if(!accepted&&j.productionVersion==='v3'){
   if(fallbackCount>=maxFallbacks)throw new Error('Short '+(shortIndex+1)+': limite de cenas de reserva excedido ('+fallbackCount+'/'+scenes.length+'). Cenas anteriores preservadas.');
   const code=generateFallbackSceneCode(scene,true);validateMotionCode(code,scene);
   accepted={intent:'Safe vertical reserve',beats:[{start:0,end:scene.durationInFrames/scene.fps,action:'Media with restrained contextual treatment'}],handoff:{opening:'direct cut',persistentElements:[],endState:'media holds',motionVector:'deliberate cut',nextOpportunity:'resume authored scene'},code,fallback:true};
   await writeFile(filename,JSON.stringify(accepted,null,2));await writeFile(path.join(folder,`Scene${scene.index}.tsx`),code);
   fallbackCount++;j.shorts.visualFallbacks=[...new Set([...(j.shorts.visualFallbacks||[]),shortIndex+':'+scene.id])];
   log(j,'Short '+(shortIndex+1)+', cena '+(scene.index+1)+': reserva visual local após cinco tentativas.');
  }
  previousHandoff=accepted.handoff;handoffs.push(previousHandoff);
 ...
 try{
  ...
  bgmTag=`<Loop durationInFrames={1800}><Audio src={staticFile("${shortBgmRel}")} volume={${j.productionVersion==='v3'?(j.shorts?.musicNormalization?.[shortIndex]?0.075:0.045):0.06}} /></Loop>`;
 }catch{
  try{
   await access(path.join(root,'renderer/public/audio/bgm-ambient.mp3'));
