...
MAP SCENES: The parent mounts a Natural Earth country map, labels and an optional explicitly requested schematic connection. A centroid connection is NOT a measured travel path; never claim that it follows a strait, road or shipping route. DO NOT attempt to draw custom bezier route curves, arrows, or country pins in SceneX.tsx. Instead, design clean editorial typography: sleek context cards, chapter titles, or narrative stats in margins or sidebars over the transparent map canvas.
...
export function sceneUnits(manifest){
 ...
 for(const scene of units){
  ...
  const isRelevant=candidate=>(candidate.asset||candidate.map)&&(!countries.size||(candidate.countries||[]).some(country=>countries.has(country)));
