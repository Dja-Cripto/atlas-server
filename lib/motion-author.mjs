...
VISUAL BREATHING ROOM & NO TEXT CLUTTER: Do NOT put cards, text boxes, or graphics on every single scene! At least 40% of scenes MUST be clean, unobstructed cinematic footage or photography with ZERO text or at most a minimal 2-3 word location pill in a corner. When previousScene had an info card or graphic, CURRENT_SCENE should give the viewer visual breathing room. Never bombard the viewer with non-stop text card after text card.
...
export function generateCleanMediaSceneCode(scene){
 ...
 const label=scene.treatment==='label'&&scene.location?String(scene.location).replace(/"/g,'\\"'):'';
...
export default function MotionScene({scene}: {scene:any; context?:any}) {
 ...
 return <AbsoluteFill style={{overflow:'hidden',backgroundColor:'#101817'}}>
  ...
  ${label?`<div style={{position:'absolute',left:64,bottom:58,padding:'10px 16px',borderRadius:8,background:'rgba(7,20,18,.68)',color:'#f5f1e7',fontSize:25,fontWeight:650,letterSpacing:.4,opacity:interpolate(frame,[0,10,Math.max(11,duration-12),duration],[0,1,1,0],{extrapolateLeft:'clamp',extrapolateRight:'clamp'})}}>${label}</div>`:''}
 </AbsoluteFill>;
}`;
}
...
export async function authorMotion(s,j,manifest,{root,log,repairError,repairSceneIndexes=[]}){
 ...
 for(const scene of scenes){
  ...
  if(scene.asset&&['clean','label'].includes(scene.treatment)&&!repairThisScene){
 ...
 const children=scenes.map(scene=>`<Sequence from={${scene.from}} durationInFrames={${scene.durationInFrames}}><AbsoluteFill><SceneBackdrop scene={scenes[${scene.index}]} backgroundScene={Number.isInteger(scenes[${scene.index}].backgroundIndex)?scenes[scenes[${scene.index}].backgroundIndex]:null} duration={${scene.durationInFrames}}/><Scene${scene.index} scene={scenes[${scene.index}]} context={contexts[${scene.index}]} /></AbsoluteFill></Sequence>`).join('');
    ...
    try{
     ...
     bgmTag=`<Loop durationInFrames={1800}><Audio src={staticFile("${jobBgmRel}")} volume={bgmVol} /></Loop>`;
