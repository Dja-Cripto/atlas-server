...
export function usesCleanMediaFastPath(scene,repairThisScene=false){return scene.editorialRole==='body'&&scene.asset?.kind==='video'&&['clean','label'].includes(scene.treatment)&&!narrationNeedsExplanation(scene)&&!repairThisScene;}
...
export async function authorMotion(s,j,manifest,{root,log,repairError,repairSceneIndexes=[]}){
 ...
 const scenes=withDocumentaryBookends(sceneUnits(manifest)),timeline=scenes.map(compactScene),model=s.motionModel||'glm-5.3-flash';
 ...
 let previousHandoff=null;const handoffs=[];let fallbackCount=0;let consecutiveFallbacks=0;const maxFallbacks=Math.max(2,Math.ceil(scenes.length*.05));const maxConsecutiveFallbacks=3;
 for(const scene of scenes){
  ...
  if(usesCleanMediaFastPath(scene,repairThisScene)){
   ...
   consecutiveFallbacks=0;log(j,`Cena ${scene.index+1}/${scenes.length}: mídia ${scene.treatment==='label'?'com identificação mínima':'limpa'}, sem chamada adicional ao GLM.`);
   continue;
  }
  ...
  for(let attempt=0;attempt<3;attempt++){
   ...
   if(!result){
    ...
    try{
     ...
     const chosenModel=motionModelForAttempt(model,attempt);
     const prompt=attempt===0?sceneContract+'\nSCENE_ASSIGNMENT: '+JSON.stringify(assignment):rescueContract+'\nSCENE_ASSIGNMENT: '+JSON.stringify(compactRescueAssignment(assignment));
     result=await generateJSON({...s,sceneProvider:'go',goModel:chosenModel,reasoningEffort:chosenModel==='glm-5.3-flash'?'low':undefined,timeoutMs:attempt===0?motionTimeout():90000},j,prompt);
     ...
     await writeFile(path.join(folder,'scene-'+scene.index+'-failure-'+(attempt+1)+'.json'),JSON.stringify({reason,model:motionModelForAttempt(model,attempt),seconds:Math.round((Date.now()-attemptStarted)/1000)}));
     log(j,'Cena '+(scene.index+1)+': '+reason+' em '+motionModelForAttempt(model,attempt)+'; tentativa '+(attempt+1)+'/3. '+String(err.message).slice(0,145));
     if(attempt===2)break;
   ...
   try{
    ...
    await writeFile(path.join(folder,`scene-${scene.index}-failure-${attempt+1}.json`),JSON.stringify({reason,model:motionModelForAttempt(model,attempt),detail:String(e.message).slice(0,250)}));
    ...
    log(j,`Cena ${scene.index+1}: a API respondeu, mas ${explanation}; tentativa ${attempt+1}/3.`);
  ...
  previousHandoff=accepted.handoff;handoffs.push(previousHandoff);
  j.auto.motion.completed=scene.index+1;j.auto.progress=70+Math.round((j.auto.motion.completed/scenes.length)*15);
  log(j,accepted.fallback?`Cena ${scene.index+1}/${scenes.length}: reserva visual pronta.`:`GLM programou cena ${scene.index+1}/${scenes.length} com sucesso.`);
 }
...
export async function authorShortsMotion(s,j,manifest,{root,log,shortIndex}){
 ...
 for(const scene of scenes){
  ...
  let accepted=null;
  if(!saved){
   for(let previousAttempt=1;previousAttempt<=3;previousAttempt++){
  ...
  for(let attempt=0;attempt<3;attempt++){
   ...
   if(!result){
    ...
    try{
     ...
     result=await generateJSON({...s,sceneProvider:'go',goModel:motionModelForAttempt(model,attempt),reasoningEffort:model==='glm-5.3-flash'?'low':undefined,timeoutMs:attempt===0?motionTimeout():90000},j,(attempt===0?shortsSceneContract:rescueContract)+'\nSCENE_ASSIGNMENT: '+JSON.stringify(attempt===0?assignment:compactRescueAssignment(assignment)));
     await writeFile(path.join(folder,`scene-${scene.index}-attempt-${attempt+1}.json`),JSON.stringify(result,null,2));
    }catch(err){
     if(attempt===2)throw new Error('Short '+(shortIndex+1)+', cena '+(scene.index+1)+': modelo de motion indisponível após três tentativas. Retome sem perder as cenas prontas.');
   ...
   try{
    ...
    saved=null;feedback=`Validation failed: ${e.message} Fix the defect.`;log(j,`Short ${shortIndex+1}, cena ${scene.index+1}: tentativa ${attempt+1}/3 rejeitada: ${e.message}`);
    if(attempt===2)throw new Error('Short '+(shortIndex+1)+', cena '+(scene.index+1)+': código visual inválido após três tentativas. Retome sem perder as cenas prontas.');
   }
  }
  previousHandoff=accepted.handoff;handoffs.push(previousHandoff);
