...
export async function authorMotion(s,j,manifest,{root,log,repairError,repairSceneIndexes=[]}){
 ...
 let previousHandoff=null;const handoffs=[];let fallbackCount=0;let consecutiveFallbacks=0;const maxFallbacks=Math.min(3,Math.max(1,Math.ceil(scenes.length*.05)));
 for(const scene of scenes){
  ...
  if(saved?.fallback){saved=null;log(j,`Cena ${scene.index+1}: repetindo autoria visual que tinha usado reserva.`);}
  ...
  if(!accepted){
   if(fallbackCount>=maxFallbacks||consecutiveFallbacks>=1)throw new Error('Falha recorrente na geração visual: mais de uma cena seguida ou limite de cenas de reserva excedido. Produção pausada; cenas anteriores preservadas para retomada.');
