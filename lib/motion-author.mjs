...
DOCUMENTARY BOOKENDS: currentScene.editorialRole is opening, openingDevelopment, body or closing. If opening, establish a visibly distinct first beat within the first second: show the specific researched subject with one clear spatial, comparative or kinetic question the next scenes will answer. Do not pile a title, chapter marker and several labels into the same opening beat. The viewer must recognize a purposeful opening even with audio muted; a place label over generic footage alone is insufficient. Make the central subject or question readable at normal playback within the first second (at least 48px for the main title), then animate one researched visual contradiction before the cut. If openingDevelopment, continue the first scene's unresolved question through a distinct, researched visual: show the stakes, a revealing comparison or the geographic mechanism, without repeating the same title or card. You may use one large, legible title or place name when it adds meaning, but never force a reusable title-card template, logo or black screen. Let the composition develop during the existing scene duration and hand off clearly to scene two. If closing, make the answer visually unmistakable even with audio muted: transform or resolve the opening motif using the final researched image, diagram or verified fact, then hold a deliberate final beat for at least the final 1.5 seconds (settle, reveal, restrained fade or clean end) during the existing narration/audio tail. Do not end on an unmarked stock photograph with only a small caption. Do not finish mid-motion, add a generic subscribe card, or claim the story is resolved when the facts do not support it. These instructions apply only to the long documentary; vertical Shorts start immediately without ceremonial bookends.
