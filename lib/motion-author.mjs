...
export function normalizeMotionCode(code,scene=null){
 ...
 let normalized=code
  ...
  .replace(/\bstyle=\{\{\s*\}\}/g,'')
  .replace(/\[\s*(-?\d+(?:\.\d+)?(?:\s*,\s*-?\d+(?:\.\d+)?)+)\s*\]/g,(match,body)=>{
   const parts=body.split(',').map(x=>Number(x.trim()));
   if(parts.some(p=>!Number.isFinite(p)))return match;
   let changed=false;
   ...
   return changed?`[${parts.join(', ')}]`:match;
  })
 ...
 while(interIdx<normalized.length){
  const match=normalized.slice(interIdx).match(/\binterpolate\s*\(/);
  ...
  if(args.length>=4&&!args[1].startsWith('[')&&args[3].startsWith('[')){
   ...
   interOut+=`interpolate(${val}, [${inMin}, ${inMax}], ${outRange}${opts?', '+opts:''})`;
  }else if(args.length===4&&!args[2].startsWith('[')&&!args[3].startsWith('[')){
   interOut+=`interpolate(${args[0]}, ${args[1]}, [${args[2]}, ${args[3]}])`;
   ...
   if(subArgs.length===2&&(subArgs[1].startsWith('{')||subArgs[1].includes('easing')||subArgs[1].includes('extrapolate'))){
    interOut+=`interpolate(${args[0]}, ${args[1]}, ${subArgs[0]}, ${subArgs[1]})`;
   }else{
    interOut+=`interpolate(${inner})`;
   }
  }else{
   interOut+=`interpolate(${inner})`;
  }
...
export function validateMotionCode(code,scene=null){
 ...
 const collectConstants=node=>{if(ts.isVariableDeclaration(node)&&ts.isIdentifier(node.name)&&node.initializer&&ts.isNumericLiteral(node.initializer)&&node.parent.flags&ts.NodeFlags.Const)numericConstants.set(node.name.text,Number(node.initializer.text));ts.forEachChild(node,collectConstants);};
...
export function sceneUnits(manifest){
 const units=manifest.scenes.map((source,index)=>{
  const from=Math.round(source.start*30);
  const end=index===manifest.scenes.length-1?Math.ceil(manifest.duration*30):Math.round(source.end*30);
  return {...source,index,from,originalStart:source.start,originalEnd:source.end,start:0,end:(end-from)/30,durationInFrames:end-from,fps:30,title:manifest.title};
 });
...
export function generateCleanMediaSceneCode(scene){
 ...
 const variant=Math.abs(Number(scene.index)||0)%5;
 const styles=[
  "left:64,bottom:58,padding:'11px 18px',borderRadius:9,background:'rgba(7,20,18,.68)',fontSize:38,fontWeight:700",
  "left:72,top:70,padding:'8px 0 8px 24px',borderLeft:'5px solid #e05b42',background:'transparent',fontSize:30,fontWeight:750,letterSpacing:'.12em',textTransform:'uppercase'",
  "left:'50%',bottom:72,transform:'translateX(-50%)',padding:'8px 22px',background:'rgba(5,14,18,.46)',fontSize:50,fontFamily:'Georgia, serif',fontWeight:500,letterSpacing:'.02em'",
  "right:70,top:74,padding:'10px 16px',borderTop:'2px solid #f0c675',background:'rgba(6,18,22,.58)',fontSize:28,fontWeight:700,letterSpacing:'.08em'",
  "right:68,bottom:60,padding:'9px 15px',borderRadius:999,background:'rgba(7,20,18,.72)',fontSize:27,fontWeight:650,letterSpacing:'.03em'"
 ];
 const labelStyle=styles[variant];
...
export default function MotionScene({scene}: {scene:any; context?:any}) {
 ...
 return <AbsoluteFill style={{overflow:'hidden',backgroundColor:'#101817'}}>
  ...
  ${safeLabel?`<div style={{position:'absolute',maxWidth:'72%',boxSizing:'border-box',boxShadow:'0 10px 32px rgba(0,0,0,.18)',color:'#f5f1e7',lineHeight:1.04,opacity:labelOpacity,${labelStyle},marginTop:labelLift}}>${safeLabel}</div>`:''}
 </AbsoluteFill>;
}`;
}
 ...
 if(!visualBible){
  ...
  for(let attempt=0;attempt<3;attempt++){
   try{
    ...
    log(j,`GLM: tentativa ${attempt+1}/3 de direção global falhou (${err.message}). Retentando...`);
   }
  }
  if(!visualBible)throw Error('Não foi possível gerar a direção visual global.');
 }else visualBible=normalizeVisualBible(visualBible,scenes)||visualBible;
 validateVisualBible(visualBible,scenes);
 log(j,`GLM (${model}): direção global pronta; iniciando programação cena por cena.`);
...
- Apply continuous subtle Ken Burns (scale 1.02 to 1.08) for life.
PACING:
