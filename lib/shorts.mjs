...
export async function automaticShorts(s,j,{root,dir,log}){
 ...
 const longCache=await readJSON(path.join(longFolder,'media-cache.json'))||{};
  ...
  const shortMusic=selectBestMusic({title:curiosity.title,script:shortScriptText,isShort:true});
  ...
  // 2c. Transcribe
  stage('transcription',`Short ${shortNum}/5: transcrevendo para sincronização.`);
  ...
  try{
   ...
   const serveUrl=await bundle({entryPoint,publicDir:path.join(root,'renderer/public')});
   const composition=await selectComposition({serveUrl,id:'AutomaticShort'});
   ...
   for(const index of indexes){
    ...
    await renderStill({composition,serveUrl,frame,output:null,imageFormat:'jpeg',scale:.25});
   }
   log(j,`Short ${shortNum}: composição validada.`);
  }catch(error){
   log(j,`Short ${shortNum}: validação com aviso — ${String(error.message).slice(0,200)}.`);
  }
