...
export async function automaticShorts(s,j,{root,dir,log}){
 ...
 const longCache=await readJSON(path.join(longFolder,'media-cache.json'))||{};
  ...
  const partialPath=`${videoPath}.partial`;
   ...
   const rendered=await renderShortMP4(s,j,i,{root,dir,log});
  ...
  log(j,`Todos os ${maxShorts} Short(s) gerados e renderizados com 100% de sucesso!`);
 }
 ...
 export async function renderShortMP4(s,j,shortIndex,{root,dir,log}){
  ...
  if(existsSync(videoPath)){
   ...
   if(stats&&stats.size>50*1024){
    ...
    if((await stat(partialPath)).size<=50*1024)throw Error(`MP4 do Short ${shortNum} inválido.`);
  ...
  try{
   ...
   return {videoPath,url:`/shorts/${j.id}/short_${shortNum}/short_${shortNum}.mp4`};
  }catch(error){await rm(partialPath,{force:true}).catch(()=>{});throw error;
  }finally{
