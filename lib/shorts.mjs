 ...
 export async function renderShortMP4(s,j,shortIndex,{root,dir,log,store}){
  ...
  try{
   ...
   onProgress:({progress,renderedFrames})=>{
    ...
    j.auto.liveStatus=`Short ${shortNum}${totalShorts>1?'/'+totalShorts:''}: renderizando ${pct}%${frameInfo} · 4 núcleos`;
    ...
    if(pct>=last+10||pct===100){last=pct;log(j,`Short ${shortNum}${totalShorts>1?'/'+totalShorts:''}: renderização ${pct}%${frameInfo}.`);}
   }
   });
