...
export async function automaticShorts(s,j,{root,dir,log,store}){
 ...
 const longCache=await readJSON(path.join(longFolder,'media-cache.json'))||{};
  ...
  for(let i=nextIndex;i<nextIndex+1;i++){
   ...
   if(!shortScriptText){
    shortScriptText=await providers.shortScript(s,curiosity,j);
   ...
   if(existsSync(shortMusicSource)){
    try{
     const shortBgm=path.join(shortPublicFolder,'bgm.mp3');
     await copyFile(shortMusicSource,shortBgm);
     if(j.productionVersion==='v3'){
      try{j.shorts.musicNormalization=j.shorts.musicNormalization||{};j.shorts.musicNormalization[i]=await normalizeBgmFile(shortBgm);log(j,'Short '+shortNum+': trilha equilibrada por arquivo.');}
      catch(error){log(j,'Short '+shortNum+': normalização da trilha indisponível ('+String(error.message).slice(0,100)+').');}
     }
     await copyFile(shortBgm,path.join(shortDataFolder,'bgm.mp3'));
    }catch{}
   }
   ...
   if(!plan){
    ...
    const directedPrompt=j.productionVersion==='v3'?v3DirectionPrompt({title:curiosity.title,script:shortScriptText,research:j.research,previous:[],batch:slots,isShort:true}):prompt;
    let response;
   ...
   const composed=composeContinuity(enforceVisualBreathing(review.scenes,duration,{version:j.productionVersion||'v2'}));
   ...
   try{
    ...
    for(const index of indexes){
     ...
     try{
      ...
      const fallbackCode=await recoverScene(s,j,scene,{folder:shortFolder,error,isShort:true,shortIndex:i,log});
      if(j.productionVersion==='v3'&&(j.shorts.visualFallbacks||[]).filter(id=>String(id).startsWith(i+':')).length>Math.max(1,Math.ceil(scenes.length*.05)))throw Error('Short '+shortNum+': limite de cenas de reserva excedido; cenas prontas preservadas.');
