 ...
 */
export function selectBestMusic({ title = '', script = '', research = {}, isShort = false, category = '', avoidPath = '', seed = '' }) {
  ...
  let pool = allTracks.filter(t => t.category === targetCategory && (targetCategory === 'funny_quirky' ? true : t.documentarySuitable));
  if (pool.length === 0) {
    pool = allTracks.filter(t => t.documentarySuitable);
  }
  if (pool.length === 0) {
    pool = allTracks;
  }
...
export function getAllTracks(rootDir = process.cwd()) {
  const baseDir = path.join(rootDir, 'Musicas');
  ...
  for (const cat of cats) {
    ...
    for (const f of files) {
      ...
      const isBeatHeavy = EXCLUDED_BEAT_TRACKS.has(f);
