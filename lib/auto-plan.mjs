...
export function enforceVisualBreathing(plan,duration){
 ...
 let composedSeconds=0,previousWasComposed=false;
 return plan.map((scene,index)=>{
  if(!['footage','photo'].includes(scene.kind))return {...scene,treatment:'composed',label:''};
  ...
  let label=treatment==='label'?(scene.label||scene.location||scene.heading||''):'';
  if(scene.asset?.representationRole==='contextual'&&label===scene.location)label=scene.heading||'';
