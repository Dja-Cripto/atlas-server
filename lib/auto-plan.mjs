...
export function normalizeVisualTreatment(treatment,kind){
 ...
 if(['map','chart','title','photo'].includes(kind))return 'composed';
...
export function enforceVisualBreathing(plan,duration){
 ...
 const videoCompositionBudget=videoSeconds*(duration<=65?.35:.30);
 let composedVideoSeconds=0,previousWasComposedVideo=false,cleanStreak=0;
 return plan.map((scene,index)=>{
  const isStill=scene.kind==='photo'||scene.asset?.kind==='image';
  if(isStill){
   previousWasComposedVideo=false;cleanStreak=0;
   return {...scene,treatment:'composed',label:'',layout:scene.layout||'full'};
  }
  if(scene.kind!=='footage'){previousWasComposedVideo=false;cleanStreak=0;return {...scene,treatment:'composed',label:''};}
  ...
  if(treatment==='composed'&&(previousWasComposedVideo||composedVideoSeconds+seconds>videoCompositionBudget))treatment='clean';
  if(treatment==='clean'&&cleanStreak>=2)treatment='label';
  if(treatment==='composed')composedVideoSeconds+=seconds;
  previousWasComposedVideo=treatment==='composed';
