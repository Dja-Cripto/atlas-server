...
export function enforceVisualBreathing(plan,duration,{version='v2'}={}){
 if(version==='v3')return plan.map((scene,index)=>{
  const isStill=scene.kind==='photo'||scene.asset?.kind==='image';
  if(isStill)return {...scene,treatment:'composed',motionStyle:'auto',label:''};
  if(scene.kind!=='footage')return {...scene,treatment:'composed',label:''};
  const bookend=index===0||index===plan.length-1;
  const opening=Number(scene.start||0)<Math.min(30,duration);
  const explanatory=narrationNeedsExplanation(scene)||Boolean(scene.diagramIntent||scene.mediaBackedExplanation||scene.mapDiversified);
  const composed=bookend||explanatory||(opening&&normalizeVisualTreatment(scene.treatment,scene.kind)==='composed');
  const treatment=composed?'composed':normalizeVisualTreatment(scene.treatment,scene.kind)==='label'?'label':'clean';
  return {...scene,treatment,label:treatment==='label'?String(scene.label||scene.location||scene.heading||'').slice(0,60):'',caption:treatment==='composed'?scene.caption:'',layout:treatment==='composed'?scene.layout:'full'};
 });
