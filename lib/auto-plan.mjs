...
export function enforceExplanatoryScenes(plan,title){
 ...
 if(!abstract||plan.length<7)return plan;
 const count=plan.filter(scene=>scene.kind==='diagram'||scene.kind==='chart').length;
 if(count>=2)return plan;
 const targets=[Math.round(plan.length*.25),Math.round(plan.length*.65)];
 const output=plan.map(scene=>({...scene}));
 for(const target of targets.slice(0,2-count)){
