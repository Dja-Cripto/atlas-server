...
export function enforceVisualBreathing(plan,duration){
 ...
 const candidates=plan.map((scene,index)=>({scene,index,seconds:Math.max(0,scene.end-scene.start)}))
  .filter(({scene})=>scene.kind==='footage'&&normalizeVisualTreatment(scene.treatment,scene.kind)==='composed')
  .sort((a,b)=>Number(priority(b.scene))-Number(priority(a.scene))||a.index-b.index);
 ...
 return plan.map((scene,index)=>{
  ...
  if(treatment==='composed'&&!selected.has(index))treatment='clean';
