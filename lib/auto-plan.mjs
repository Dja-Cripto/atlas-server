...
export function validateDirection(slot,d,research){
 ...
 const result={...slot,kind,treatment,heading,caption:treatment==='composed'?text(d.caption,130):'',query:text(d.query,100),location:text(d.location,80),countries:Array.isArray(d.countries)?d.countries.filter(x=>typeof x==='string'&&x.length<60).slice(0,5):[],routes:d.routes===true,layout:['full','split','overlay'].includes(d.layout)?d.layout:'full'};
...
export function enforceVisualBreathing(plan,duration){
 ...
 const composedBudget=mediaSeconds*(duration<=65?.30:.25);
 const labelBudget=mediaSeconds*.20;
 let composedSeconds=0,labelSeconds=0,previousWasComposed=false;
 return plan.map(scene=>{
  if(!['footage','photo'].includes(scene.kind))return {...scene,treatment:'composed'};
  const seconds=Math.max(0,scene.end-scene.start);
  let treatment=normalizeVisualTreatment(scene.treatment,scene.kind);
  if(treatment==='composed'&&(previousWasComposed||composedSeconds+seconds>composedBudget))treatment='clean';
  if(treatment==='label'&&(!scene.location||labelSeconds+seconds>labelBudget))treatment='clean';
  if(treatment==='composed')composedSeconds+=seconds;
  if(treatment==='label')labelSeconds+=seconds;
  previousWasComposed=treatment==='composed';
  return {...scene,treatment,caption:treatment==='composed'?scene.caption:'',layout:treatment==='composed'?scene.layout:'full'};
 });
}
