...
export function enforceVisualBreathing(plan,duration){
 ...
 const selected=new Set(plan.flatMap((scene,index)=>scene.kind==='footage'&&narrationNeedsExplanation(scene)?[index]:[]));let composedVideoSeconds=[...selected].reduce((sum,index)=>sum+Math.max(0,plan[index].end-plan[index].start),0);
...
export function diversifyVisualPlan(plan,duration){
 ...
 const seenMaps=new Set(),seenIntents=new Set();let acceptedMaps=0,mapSeconds=0;
 const diversified=plan.map((source,index)=>{
  if(index===0&&duration>65&&source.kind!=='footage'&&!source.mapIntent&&!source.editorialReason){
  ...
  const intent=String(source.mapIntent||'').trim().toLowerCase();
  const advancesStory=Boolean(intent&&!seenIntents.has(signature+'|'+intent));
  if(!advancesStory&&(repeatsMap||consecutive||exceedsCount||exceedsRuntime)){
  ...
  if(intent)seenIntents.add(signature+'|'+intent);
...
export function enforceExplanatoryScenes(plan,title,duration=60){
 ...
 if(diagrams.length>maxDiagrams){
  ...
  for(const index of diagrams){
   if(keep.has(index)||output[index].editorialReason)continue;
