...
export async function processNextTopicInQueue(store, geminiKey, runProductionFn) {
  const dir = store.dir;
  const data = loadTopicsData(dir);
  const pending = data.queue.filter(t => t.status === 'pending').sort((a, b) => a.order - b.order);
