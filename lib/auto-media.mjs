...
async function probeDimensions(file){
 return new Promise((resolve,reject)=>{
  const child=spawn(process.env.FFPROBE_BIN||'ffprobe',['-v','error','-select_streams','v:0','-show_entries','stream=width,height','-of','json',file],{windowsHide:true,stdio:['ignore','pipe','pipe']});
  let out='',err='';child.stdout.on('data',chunk=>out+=chunk);child.stderr.on('data',chunk=>err+=chunk);child.on('error',reject);
  child.on('exit',code=>{if(code!==0)return reject(Error('Mídia inválida: '+err.slice(-160)));try{const stream=JSON.parse(out).streams?.[0];if(!stream)throw Error('Sem imagem');resolve({width:Number(stream.width),height:Number(stream.height)});}catch(e){reject(e);}});
 });
}
...
export async function transcodeVideo(inputPath,outputPath){
 return new Promise((resolve,reject)=>{
  ...
  const args=['-y','-hide_banner','-loglevel','error','-i',inputPath,'-vf',"scale='min(1920,iw)':-2:flags=bicubic,fps=30",'-c:v','libx264','-preset','ultrafast','-crf','22','-g','30','-keyint_min','30','-pix_fmt','yuv420p','-an',outputPath];
...
export async function sourceScene(s,j,scene,folder,cache,excluded=new Set()){
 ...
 const result={...dimensions,src:`${relPrefix?relPrefix+'/':''}${name}`,kind:isVideo?'video':'image',trimStart:0,duration:selected.duration||null,representationRole:selected.representationRole||'exact-location',credit:{author:selected.author,source:selected.source,url:selected.url,license:selected.license,licenseUrl:selected.licenseUrl||'',reason:selected.selectionReason}};
