...
export const meetsFullHd=({width,height}={})=>Number(width)>=1920&&Number(height)>=1080;
...
export function candidatePool(scene,candidates,limit=6){
 const eligible=candidates.filter(candidate=>candidate.files?.length?candidate.files.some(file=>meetsFullHd(file)):!candidate.width||!candidate.height||meetsFullHd(candidate));
 const exact=eligible.filter(candidate=>hasLocation(candidate,scene.location));
 const exactIds=new Set(exact.map(candidate=>candidate.source+':'+candidate.id));
 const contextual=eligible.filter(candidate=>!exactIds.has(candidate.source+':'+candidate.id)&&['Pexels','Pixabay'].includes(candidate.source));
...
export async function generateIllustrativeScene(s,j,scene,folder){
 ...
 const name='illustration-'+createHash('sha256').update(scene.id+'|'+scene.narration).digest('hex').slice(0,20)+'.jpg';
 ...
 return {...dimensions,src:`${relPrefix?relPrefix+'/':''}${name}`,kind:'image',duration:null,representationRole:'illustrative',credit:{source:'Gemini',reason:'Ilustração editorial gerada; não retrata o local exato nem constitui evidência documental.'}};
}
export function reusableMediaCandidates(scene,media,cache,excluded=new Set()){
 ...
 return (media||[]).filter(candidate=>samePlaceUrls.has(candidate.url)&&!excluded.has(candidate.url)&&(!candidate.files?.length||Number(candidate.duration)>=scene.end-scene.start)).sort((a,b)=>overlap(b)-overlap(a)).slice(0,12);
}
export async function sourceScene(s,j,scene,folder,cache,excluded=new Set()){
 const key=[scene.id,scene.query,scene.location,scene.kind,scene.openingVideoRequired?'opening-video':''].join('|');
 if(cache[key]&&meetsFullHd(cache[key])&&(!scene.openingVideoRequired||cache[key].kind==='video')&&(cache[key].kind==='image'||cache[key].duration>=scene.end-scene.start))return cache[key];
 ...
 const usedUrls=new Set([...excluded,...Object.entries(cache).filter(([cachedKey])=>cachedKey!==key).map(([,asset])=>asset?.credit?.url).filter(Boolean)]);
 let selected=null;try{selected=JSON.parse(await readFile(selectionFile,'utf8'));if(usedUrls.has(selected.url))selected=null;}catch{}
 ...
 if(scene.reuseExistingMedia&&!selected)selected=await choose(s,scene,reusableMediaCandidates(scene,j.media,cache,excluded),folder,new Set(excluded));
 if(scene.openingVideoRequired&&selected&&!selected.files?.length)selected=null;
 ...
 if(!selected&&(scene.kind==='photo'||isHistorical&&!scene.openingVideoRequired)){
 ...
 if(!selected&&!scene.openingVideoRequired){
  const archiveSearches=await Promise.allSettled([commons,libraryOfCongress].flatMap(search=>variants.map(async query=>{const items=await search(query);searched('archive-photo',1,items.length);return items.map(item=>({...item,queries:[query],searchPage:1}));})));
  const archiveItems=unique(archiveSearches.flatMap(result=>result.status==='fulfilled'?result.value:[]));remember(j,archiveItems,scene);selected=await choose(s,scene,archiveItems,folder,usedUrls);
 ...
 const name=createHash('sha256').update(url+(isVideo?'|v2|'+Math.ceil(Math.max(1,scene.end-scene.start)+1):'')).digest('hex').slice(0,20)+ext;
 const destPath=path.join(folder,name);
 const clipDuration=Math.max(1,scene.end-scene.start)+1;
 ...
 const result={...dimensions,src:`${relPrefix?relPrefix+'/':''}${name}`,kind:isVideo?'video':'image',trimStart:0,duration:isVideo?(dimensions.duration||preparedDuration):null,representationRole:selected.representationRole||'exact-location',credit:{author:selected.author,source:selected.source,url:selected.url,license:selected.license,licenseUrl:selected.licenseUrl||'',reason:selected.selectionReason}};
