...
export async function sourceScene(s,j,scene,folder,cache){
 ...
 try{await readFile(path.join(folder,name));}catch{const data=await cachedDownload(folder,url,120_000_000);await writeFile(path.join(folder,name),data.bytes);}
