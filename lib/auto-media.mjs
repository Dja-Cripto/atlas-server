...
export async function sourceScene(s,j,scene,folder,cache,excluded=new Set()){
 const key=[scene.id,scene.query,scene.location,scene.kind].join('|');
 if(cache[key]&&meetsFullHd(cache[key])&&(cache[key].kind==='image'||cache[key].duration>=scene.end-scene.start))return cache[key];
