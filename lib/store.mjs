...
export function createStore(dir){
 ...
 return {
 dir,
 create:(data={})=>{const id=data.id||randomUUID();const j={id,title:String(data.title||'').trim(),notes:String(data.notes||data.description||'').trim(),minutes:Number(data.minutes)||5,productionVersion:'v3',editorialVersion:'observant-dry-v1',generateShorts:Boolean(data.generateShorts),shortsCount:data.generateShorts?5:0,status:'draft',createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),completed:[],events:[{at:new Date().toISOString(),message:'Projeto criado. Pronto para iniciar a produção.'}],current:null,approvedMedia:[],...data};db.prepare('INSERT INTO records VALUES (?,?) ON CONFLICT(id) DO UPDATE SET value=excluded.value').run(j.id,JSON.stringify(j));return j;},
