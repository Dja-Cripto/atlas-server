...
export function buildGeographicMap(scene,world){
 ...
 const narration=countryKey(scene.narration||'');
 const features=(scene.countries||[]).map((name,index)=>{
  const key=countryKey(name);
  const spokenLabel=Boolean(key&&(' '+narration+' ').includes(' '+key+' '));
  ...
  if(isFrenchGuiana(key)){
   ...
   if(france&&france.geometry?.coordinates?.[0]){
    return {
      ...
      properties:{name:'French Guiana',ADMIN:'French Guiana',focal:index===0,requestedName:name,spokenLabel}
    };
   }
  }
  ...
  if(isMetroFrance(key)||(key==='france'&&hasFG)){
   ...
   if(france&&france.geometry?.coordinates?.length>1){
    ...
    return {
      ...
      properties:{name:isMetroFrance(key)?'Metropolitan France':'France',ADMIN:'France',focal:index===0,requestedName:name,spokenLabel}
    };
   }
  }
  ...
  return {type:'Feature',geometry:found.geometry,properties:{name:found.properties.ADMIN||found.properties.NAME,focal:index===0,requestedName:name,spokenLabel}};
 });
