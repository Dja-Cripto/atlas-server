...
export const ContextMap:React.FC<{map:FeatureCollection;duration:number}>=({map,duration})=>{
 ...
 useEffect(()=>{let live=true;fetch(staticFile('auto/world-background.json')).then(r=>{if(!r.ok)throw Error('Geographic basemap unavailable');return r.json();}).then(data=>{if(live){setWorld(data);continueRender(handle);}}).catch(cancelRender);return()=>{live=false;};},[handle]);
  const geometry=useMemo(()=>{
   ...
   let route=null;
   if(focus.length>=2){
    const p1=focus[1].point,p2=focus[0].point;
  ...
  return <AbsoluteFill style={{background:'#0c1922',overflow:'hidden'}}>
   <svg width={1920} height={1080}>
    ...
    <rect width={1920} height={1080} fill="url(#map-grid)"/>
    <g transform={`translate(960 540) scale(${zoom}) translate(-960 -540)`}>
     ...
     {geometry.route&&(()=>{
      const rProgress=interpolate(f,[15,Math.min(65,duration*.75)],[0,1],{extrapolateLeft:'clamp',extrapolateRight:'clamp',easing:Easing.out(Easing.cubic)});
      const t=rProgress;
     ...
     {geometry.focus.map((feature,i)=>{
      ...
      return <g key={`f-${i}`}>
       <path d={feature.path} fill={feature.focal?'#d49a42':'#2a5a54'} fillOpacity={feature.focal?.85:.7} stroke={feature.focal?'#ffd485':'#52b2a4'} strokeWidth={feature.focal?2.5:1.8}/>
       <g opacity={fade}>
        <circle cx={pt[0]} cy={pt[1]} r={8} fill={feature.focal?'#ffd485':'#52b2a4'} fillOpacity={.35}/>
        <circle cx={pt[0]} cy={pt[1]} r={3.5} fill="#fff"/>
        <text x={pt[0]} y={pt[1]-18} fill="#ffffff" stroke="#0c1922" strokeWidth={5} paintOrder="stroke" textAnchor="middle" fontSize={26} fontWeight={700} letterSpacing={2} style={{fontFamily:'"Helvetica Neue", Arial, sans-serif'}}>{feature.name.toUpperCase()}</text>
       </g>
      </g>;
     })}
    </g>
   </svg>
   <div style={{position:'absolute',left:80,bottom:35,color:'rgba(217,238,228,0.7)',fontSize:17,letterSpacing:1}}>Natural Earth · Geographic boundaries</div>
  </AbsoluteFill>;
};
